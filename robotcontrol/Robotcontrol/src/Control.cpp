...
void Control::compute() {
    ...
    rollPID.Compute();
    motorOutput.speedLeft = rollOutput - input.steerProportion * CONTROL_MAX_STEER_STEP16_OFFSET;
    motorOutput.speedRight = -rollOutput + input.steerProportion * CONTROL_MAX_STEER_STEP16_OFFSET;
    cycleNo++;
    if (cycleNo >= 1000) {
        cycleNo = 0;
    }
